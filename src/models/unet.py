...
def unet(input_shape, kernel_size, filters, dropout_rate, output_ch):
    gpu1, gpu2, gpu3 = '/gpu:0', '/gpu:0', '/gpu:0'
    with tf.device(gpu1):
        inputs = Input(input_shape, name='input')
        conv1 = UNetUnit(filters, kernel_size, dropout_rate)(inputs)
        pool = MaxPooling2D(pool_size=(2,2))(conv1)
        conv2 = UNetUnit(2*filters, kernel_size, dropout_rate)(pool)
        pool = MaxPooling2D(pool_size=(2,2))(conv2)
        conv3 = UNetUnit(4*filters, kernel_size, dropout_rate)(pool)
        pool = MaxPooling2D(pool_size=(2,2))(conv3)
    with tf.device(gpu2):
        conv4 = UNetUnit(8*filters, kernel_size, dropout_rate)(pool)
        pool = MaxPooling2D(pool_size=(2,2))(conv4)
        conv5 = UNetUnit(16*filters, kernel_size, dropout_rate)(pool)
        transpose = UNetUp()(conv4, conv5)
        conv = UNetUnit(8*filters, kernel_size, dropout_rate)(transpose)
    with tf.device(gpu3):
        transpose = UNetUp()(conv3, conv)
        conv = UNetUnit(4*filters, kernel_size, dropout_rate)(transpose)
        transpose = UNetUp()(conv2, conv)
        conv = UNetUnit(2*filters, kernel_size, dropout_rate)(transpose)
        transpose = UNetUp()(conv1, conv)
        conv = UNetUnit(filters, kernel_size, dropout_rate)(transpose)
        out = Conv2D(output_ch, (1,1), activation='softmax', name='mask', kernel_initializer=KERNEL_INITIALIZER)(conv)
        
    model = Model(inputs=inputs, outputs=out)
    return model

def unet_multitask(input_shape, kernel_size, filters, dropout_rate, output_ch):
    ...
    with tf.device('/gpu:1'):
        ...
        transpose = UNetUp()(conv1, conv)
        conv = UNetUnit(filters, kernel_size, dropout_rate=dropout_rate)(transpose)
        out2 = Conv2D(2, (1,1), activation='softmax', name='bnd')(conv)

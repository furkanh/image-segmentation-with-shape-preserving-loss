...
class Group1Block:
    ...
    def __call__(self, layer, resize):
        B = Conv2D(self.filters, self.kernel_size, activation=ACTIVATION)(layer)
        B = Conv2D(self.filters, self.kernel_size, activation=ACTIVATION)(B)
        B = MaxPooling2D(pool_size=(2,2))(B)
        inter = Conv2D(self.filters, self.kernel_size, activation=ACTIVATION)(resize)
...
class Group2:
    ...
    def __call__(self, layer):
        ...
        out = Conv2D(self.filters, self.kernel_size, activation=ACTIVATION)(out)
        out = Conv2D(self.filters, self.kernel_size, activation=ACTIVATION)(out)
        out = Conv2DTranspose(self.filters, self.kernel_size, strides=(2,2), output_padding=1)(out)
        out = Conv2D(self.filters, self.kernel_size, activation=ACTIVATION)(out)
        out = Conv2D(self.filters, self.kernel_size, activation=ACTIVATION)(out)
        out = Conv2DTranspose(self.filters, self.kernel_size)(out)
        return out
...
def micronet508(filters, dropout_rate, output_ch):
    kernel_size=(3,3)
    size=508
    ...
    with tf.device('/gpu:0'):
        ...
        B1 = Group1Block(filters, kernel_size)(inputs, resize1)
        B2 = Group1Block(2*filters, kernel_size)(B1, resize2)
        B3 = Group1Block(4*filters, kernel_size)(B2, resize3)
        B4 = Group1Block(8*filters, kernel_size)(B3, resize4)
